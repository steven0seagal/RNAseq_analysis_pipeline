process FASTP {
    ...
    def touch_reads         = (is_single_output && !save_trimmed_fail && !save_merged) ? "echo '' | gzip > ${prefix}.fastp.fastq.gz" : ""
    def touch_merged        = (!is_single_output && save_merged) ? "echo '' | gzip > ${prefix}.merged.fastq.gz" : ""
    def touch_fail_fastq    = (!is_single_output && save_trimmed_fail) ? "echo '' | gzip > ${prefix}.paired.fail.fastq.gz && echo '' | gzip > ${prefix}_1.fail.fastq.gz && echo '' | gzip > ${prefix}_2.fail.fastq.gz" : save_trimmed_fail ? "echo '' | gzip > ${prefix}.fail.fastq.gz" : ""
    """
    $touch_reads
    $touch_fail_fastq
    $touch_merged
